...
export default function NovoAnuncio() {
  ...
  const [form,setForm]=useState({title:'',description:'',price:'',neighborhood:'',contactWhatsApp:'',hours:'',categoryId:'', images:[]});
  ...
  async function submit(e){ e.preventDefault(); setErr('');
    ...
    const r = await fetch('/api/ads',{method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)});
